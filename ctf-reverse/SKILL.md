...
- [recent-trends-2025-2026.md](recent-trends-2025-2026.md) - Recent reverse trends: non-ELF targets, custom formats, browser extensions, HTML/JS challenges, Minecraft datapacks, VM bytecode workflows, and instrumentation-first triage
...
1. **Classify the artifact first** - do not assume ELF/PE. Check whether it is ZIP/XPI/APK/WASM/HTML/JS/custom data before opening the decompiler.
2. **Start with strings and container inspection** - many easy or medium challenges leak flags, keys, routes, or format markers early.
3. **Try ltrace/strace or equivalent runtime tracing** - dynamic analysis often reveals flags without full reversing.
4. **Try Frida hooking** - hook strcmp/memcmp, VM handlers, or decode routines to capture expected values without reversing every block.
5. **If you see a dispatcher + blob, build a disassembler early** - modern rev often uses custom VM bytecode or a homegrown DSL.
6. **If the format is custom, reverse the writer/save path** - reconstruct on-disk structures, metadata tables, and serialized trees from controlled examples.
7. **Try angr / Triton / Z3 when the verifier is constraint-friendly** - symbolic execution still solves many checkers automatically.
8. **Try Qiling or emulation when architecture, environment, or anti-debug is the blocker**.
9. **Automate manual processes** via scripting (r2pipe, Frida, angr, Python).
10. **Validate assumptions** by comparing decompiler outputs and runtime state.

For the recent nonstandard-target workflow, read [recent-trends-2025-2026.md](recent-trends-2025-2026.md) after the first file-format check.
...
If the target is not clearly a native executable, immediately check [recent-trends-2025-2026.md](recent-trends-2025-2026.md) before spending time in Ghidra.
...
1. Run a short recon pass before diving deep. Use [shared/common-triage.md](../shared/common-triage.md) when the starting point is ambiguous.
2. Write down facts, hypotheses, and reusable primitives separately. This prevents repeating failed ideas.
3. Prefer the fastest discriminating test first: verify one primitive, then escalate only when evidence supports it.
4. When handing off to another ctf-* skill, include the current evidence ledger and the exact commands already tried.

## Progress reporting

Use the compact structure in [shared/output-template.md](../shared/output-template.md) when summarizing findings, especially before a pivot or handoff.
